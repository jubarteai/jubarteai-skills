...
| `create_knowledge` | `agent_id`, `title`, `description` (dense — insight + why in 2–4 sentences), `branches[]` (min 1), `repositories[]` (min 1, normalized to lowercase bare-name slugs server-side), `refs[]?`, `tags[]?` (a few keyword labels, returned in search for triage), `paths[]?` (repo-relative files/dirs the entry describes, default `[]`), `metadata?` (allow-listed: `status`, `supersedes` — one uuid or an array of up to 20, every id validated to exist in your company, `related_ids`), `kind?` (default `"knowledge"`; `knowledge`\|`decision`\|`memory`\|`note`\|`workdone`) | `{ id }` | When you learn something reusable — continuously. **Search first** to find an entry to update; only create when none fits. |
| `get_knowledge` | `agent_id`, `id?` or `name?` (exact title, case-insensitive) | `{ entry }` — full body + `refs` + `updated_at` (pass back as `expected_updated_at` on `update_knowledge`) + verification record: `paths, needs_verification, verified_at, verified_by_seat_id, verified_by_agent_id, flagged_at, flag_source, flag_ref, flag_paths, flag_reason`, and `superseded_by` (uuid \| null — non-null means prefer the superseding entry instead; on lookup failure `superseded_by_error` instead — unknown ≠ none, retry or treat as unverified). | On the **top** `search_knowledge` hit before acting on it; or fetching by exact title. |
| `update_knowledge` | `agent_id`, `id`, plus ≥1 of `title?`, `description?`, `branches[]?`, `repositories[]?`, `refs[]?`, `tags[]?`, `metadata?` (`supersedes`: uuid \| uuid[], max 20, same validation as `create_knowledge`), `kind?`, `paths[]?` — plus optional `expected_updated_at?` (the `updated_at` from `get_knowledge`; does **not** count toward the ≥1) | `{ id, updated_at, verified_at \| verification_error }` — fresh `updated_at`; a `title`/`description` change adds `verified_at` (verified) or `verification_error` (write saved, verification failed — retry with `verify_knowledge`); neither key otherwise | Improving/reclassifying an entry instead of duplicating; keeping your workdone current. On a hot/shared entry, pass `expected_updated_at` — mismatch rejects the write with a re-fetch-and-merge error instead of silently clobbering. A content edit also verifies the entry. |
