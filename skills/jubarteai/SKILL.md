...
2. **Situational awareness** — `list_agents({ agent_id })` once to see peers and their `current_task` (incl. `paths`); the list is pre-filtered server-side to active agents (not disconnected, seen in the last 30 min). A `paths` overlap with a peer is the fast-collision signal — avoid duplicate work. Each peer's `claims[]` is also here — check it before claiming or editing a contested resource.
