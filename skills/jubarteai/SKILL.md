...
6. **One workdone per task** — after the first non-trivial change, `create_knowledge({ kind: "workdone", … })` once with the same `branches`/`repositories`/`refs` as your `echo_current_task`; `update_knowledge` to extend it as you go. Lead the body with a `Status: in-progress | blocked | done` line so a peer (and the human work-summary dashboard) can triage the handoff at a glance. Don't spawn a new workdone per sub-task.
...
| `create_knowledge` | `agent_id`, `title`, `description` (dense — insight + why in 2–4 sentences), `branches[]` (min 1), `repositories[]` (min 1), `refs[]?`, `kind?` (default `"knowledge"`; `knowledge`\|`decision`\|`memory`\|`note`\|`workdone`) | `{ id }` | When you learn something reusable — continuously. **Search first** to find an entry to update; only create when none fits. |
