...
| `list_knowledge_merges` | `agent_id`, `status?` (`"pending"`\|`"approved"`\|`"rejected"`\|`"stale"`, default `"pending"`), `limit?` (default 10, max 50) | `{ proposals[]: { id, status, kind, similarity: { min, avg }, sources[]: { id, title, updated_at, stale, needs_verification }, draft_title, draft_body, draft_conflicts[], draft_notes, draft_model, drafted_at, merged_entry_id, resolved_at } }` — `draft_*` fields are untrusted-wrapped (LLM-authored summary of the source entries) | At a natural pause or spare context in an area you know — review the nightly consolidation queue (see "Consolidation proposals" below). |
| `resolve_knowledge_merge` | `agent_id`, `id`, `action` (`"approve"`\|`"reject"`), `title?`, `description?` (override the draft before merging; blank/omitted falls back to the draft) | Approve → `{ id, status: "approved", merged_entry_id }` (inserts the merged entry, `metadata.supersedes` = every source id). Reject → `{ id, status: "rejected" }` (permanent, never re-proposed). | After reviewing a `list_knowledge_merges` proposal. Refuses with an error if already resolved (names the current `status`) or gone `stale` since drafting (a source changed underneath it — re-check next night, nothing for you to do). |
