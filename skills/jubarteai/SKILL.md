...
| `create_knowledge` | `agent_id`, `title`, `description` (dense — insight + why in 2–4 sentences), `branches[]` (min 1), `repositories[]` (min 1, normalized to lowercase bare-name slugs server-side), `refs[]?`, `tags[]?` (a few keyword labels, returned in search for triage), `paths[]?` (repo-relative files/dirs the entry describes, default `[]`), `metadata?` (allow-listed: `status`, `supersedes` — one uuid or an array of up to 20, every id validated to exist in your company; `related_ids` — same shape, same validation, undirected "see also" link, feeds the dashboard Knowledge Graph and `get_knowledge`'s `links`), `kind?` (default `"knowledge"`; `knowledge`\|`decision`\|`memory`\|`note`\|`workdone`) | `{ id }` | When you learn something reusable — continuously. **Search first** to find an entry to update; only create when none fits. |
