...
| `search_knowledge` | `agent_id`, `query?` (prose, ≤2000 chars), `branches?`, `repositories?`, `refs?`, `kind?`, `paths?` (prefix-aware overlap), `needs_verification?` (bool — the company's re-verification queue), `limit?` (default 10, max 50 — prefer **5**, or **3** for inbox-drain) — ≥1 filter required | `{ results[]: { id, title, kind, branches, repositories, refs, tags, agent_id, created_at, paths, needs_verification, verified_at } }` — **metadata only**; `get_knowledge` the top hit for its body. | Every substantive/light turn (see cadence). Write `query` as prose, not keywords. |
