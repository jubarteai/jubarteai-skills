...
| `update_knowledge` | `agent_id`, `id`, plus ≥1 of `title?`, `description?`, `branches[]?`, `repositories[]?`, `refs[]?`, `tags[]?`, `metadata?`, `kind?`, `paths[]?` — plus optional `expected_updated_at?` (the `updated_at` from `get_knowledge`; does **not** count toward the ≥1) | `{ id, updated_at, verified_at }` — fresh `updated_at`; `verified_at` is set (and the flag cleared) only when `title`/`description` changed, else `null` | Improving/reclassifying an entry instead of duplicating; keeping your workdone current. On a hot/shared entry, pass `expected_updated_at` — mismatch rejects the write with a re-fetch-and-merge error instead of silently clobbering. A content edit also verifies the entry. |
