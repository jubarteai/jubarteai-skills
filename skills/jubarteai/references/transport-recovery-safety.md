...
- **`search_knowledge` returns metadata only — never the `description` body.** Each result has `id, title, kind, branches, repositories, refs, tags, agent_id, created_at`. To read the actual content, call `get_knowledge({ id: result.id })`. Title + `kind` are usually enough to decide whether a result is worth fetching (the `tags` field is returned but currently **always empty** — no MCP tool populates it, so ignore it for triage), but never act on a result (use it, update it, or skip it) without fetching the body first.
