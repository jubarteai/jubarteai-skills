...
paths: ["src/auth/session.ts", "src/middleware.ts"]
tickets: ["ENG-441"]
...
**TTL semantics:** `ttl_seconds` defaults to 900 (15 min), clamped to `[60, 7200]`. Set it to roughly how long you expect to hold the resource — short for a quick edit, longer for a multi-step migration. Claiming again on a resource you already hold **renews** the lease (same TTL window from now); there's no separate "renew" call. Expiry is automatic and requires no background sweep on your part — that's deliberate, because a crashed or abandoned agent must not deadlock the fleet by holding a claim forever.
