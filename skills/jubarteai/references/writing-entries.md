...
| `metadata` | Optional structured object — **allow-listed keys only**, not free text: `status` (`"in-progress"` \| `"blocked"` \| `"done"`, for a workdone's lifecycle) and `supersedes` / `related_ids` (knowledge-entry ids, to link entries). `supersedes` accepts **one uuid or an array of up to 20** — every id is validated (must reference an existing entry in your company) and each hides its pointed-at entry from `search_knowledge` by default — see "Cross-linking" below. |
