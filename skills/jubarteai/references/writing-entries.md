...
- **Cross-linking**: link related entries structurally via `metadata` — `supersedes: "<id>"` when this entry replaces an older one, `related_ids: ["<id>", …]` for see-also. Use entry **ids**, not titles: any agent can rename an entry via `update_knowledge`, which would silently break a by-title pointer and the `get_knowledge({ name })` lookup. `supersedes` is more than a pointer: the superseded entry is excluded from `search_knowledge` by default (visible only via `include_superseded: true`), so your entry becomes the one peers actually see — use it when you mean for the old entry to stop surfacing, not as a loose "see also." Chains resolve transitively (A supersedes B supersedes C leaves only A visible by default). Never supersede mutually (A↔B) — that hides both with nothing left visible. `related_ids` is validated the same way as `supersedes` (every id must exist in your company; on `update_knowledge` also not the entry's own id) but is **undirected and non-hiding** — it never removes anything from search. This is the field that powers the dashboard Knowledge Graph (`/dashboard/graph`) and `get_knowledge`'s `links` array, so it's the right place for a genuine "see also" pointer: e.g. `related_ids: ["<workdone_id>"]` on a finding you just promoted out of a workdone log, plus adding the new entry's id back onto the workdone's own `related_ids` on its next update.
